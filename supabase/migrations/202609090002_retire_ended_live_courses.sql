-- Automatically archive a bookable live-lecture course once its session has
-- ended. Each occurrence is its own one-lesson course; when the live lesson's
-- phase is 'ended' (Zoom reported the end, or starts_at + 6h passed) and the
-- course carries no other real content, it is retired: offers deactivated,
-- course and live lesson set 'retired'. Payment, invoice, attendance and the
-- adci_live_classes row are kept (the admin schedule still shows the past
-- session and its attendance for its own 30-day window).
--
-- The catalogue already drops it from sale (sale_ends_at). This makes it also
-- disappear from People > Manage and the bulk-grant course picker.
begin;
...
begin
  with ended_live_courses as (
    ...
    where c.organization_id = org_id
      ...
      -- every non-retired live lesson that still has a class row has ended
      and not exists (
        select 1 from public.adci_lessons l
        join public.adci_modules m on m.id = l.module_id
  ...
  select coalesce(array_agg(id), '{}') into retired_ids from ended_live_courses;
...
-- Close the gap between a session ending and the nightly sweep: hide a live
-- course from the admin course-access surfaces as soon as its live lesson has
-- ended, not only once it is retired.
