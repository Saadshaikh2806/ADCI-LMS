...
const removeAgoraMigration = readFileSync(
  join(root, "supabase", "migrations", "202609080004_remove_agora_live.sql"),
  "utf8"
);
...
const paidLiveRules = [
  ...
  [removeAgoraMigration, "drop function if exists public.adci_authorize_agora_join", "Agora join authorisation removed"],
  [removeAgoraMigration, "delete from public.adci_live_classes where provider = 'agora'", "Existing Agora live classes retired"],
  [removeAgoraMigration, "Only Zoom Live sessions are supported", "Bookable live series is Zoom-only"],
  [adminLiveSchedule, 'Zoom Live</button>', "Separate Zoom Live creation button"],
  [adminLiveSchedule, "openBookableSeries()", "Zoom Live creation path"],
  ...
  [paidLiveRoute, 'session_provider: "zoom"', "Zoom-only live-session creation"],
