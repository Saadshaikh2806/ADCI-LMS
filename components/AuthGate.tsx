...
import { verifyActiveSession } from "../lib/supabase/session";
...
export default function AuthGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function validateSession(candidate: Session | null) {
      ...
      try {
        await verifyActiveSession(authClient, true);
      } catch (sessionError) {
        await authClient.auth.signOut({ scope: "local" });
        if (active) {
          setSession(null);
          setMessage(sessionError instanceof Error ? sessionError.message : "Unable to verify your session. Please sign in again.");
          setLoading(false);
        }
        return;
      }
  ...
  useEffect(() => {
    ...
    async function ensureStillOwner() {
      if (!active) return;
      try {
        await verifyActiveSession(client);
      } catch (sessionError) {
        if (!active) return;
  ...
  async function finishMfaSignIn() {
    ...
    try {
      await verifyActiveSession(supabase, true);
    } catch (error) {
