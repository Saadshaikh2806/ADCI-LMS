...
A bookable live-lecture course is archived automatically once its session has ended: a nightly Vercel Cron (`/api/live-sessions/retire-ended`) retires it, and the admin course-access surfaces hide it the moment the class ends. The Live schedule has a **Tidy ended** button to run it on demand. Attendance and the past session stay visible in the Live schedule for 30 days. Apply `202609090002_retire_ended_live_courses.sql` and `202609090003_archive_stranded_live_courses.sql`.

Spent live lectures are matched by the bookable-series slug (`<title>-YYYY-MM-DD-<8 hex>`), which only `adci_create_bookable_live_series` produces, so a shell left behind after its lessons were removed is archived too. Ordinary courses never match.
